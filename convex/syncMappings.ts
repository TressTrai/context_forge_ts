...
export const importFromAnchors = mutation({
  args: {
    projectId: v.id("projects"),
    provider: v.union(v.literal("github"), v.literal("gitlab")),
    repoUrl: v.string(),
  ...
  handler: async (ctx, { projectId, provider, repoUrl, branch, folder, anchors }) => {
    const userId = await getAuthUserId(ctx)
    if (!userId) throw new Error("Not authenticated")
    ...
    let mappingId: Id<"syncMappings">
    if (existing) {
      await ctx.db.patch(existing._id, { provider, repoUrl, branch, folder })
      mappingId = existing._id
    } else {
      mappingId = await ctx.db.insert("syncMappings", { projectId, userId, provider, repoUrl, branch, folder })
    }

    for (const anchor of anchors) {
      const blockId = anchor.blockId as Id<"blocks">
      ...
      if (!existingBlock) {
        await ctx.db.insert("syncBlocks", { syncMappingId: mappingId, blockId, path: anchor.path })
      }
    }
